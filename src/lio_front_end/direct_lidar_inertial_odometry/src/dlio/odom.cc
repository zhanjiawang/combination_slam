...
dlio::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle) {
  ...
  //改为四核
  this->num_threads_ = 4;  // omp_get_max_threads();
  ...
  this->imu_sub = this->nh.subscribe("imu", 1000, &dlio::OdomNode::callbackImu,
                                     this, ros::TransportHints().tcpNoDelay());
  this->livox_sub =
      this->nh.subscribe("livox", 1, &dlio::OdomNode::callbackLivox, this,
  ...
  this->livox_pub =
      this->nh.advertise<sensor_msgs::PointCloud2>("livox2dlio", 1, true);

  this->perception_odom_pub =
      this->nh.advertise<nav_msgs::Odometry>("/front_end_odom", 1, true);
  this->perception_cloud_pub =
      this->nh.advertise<sensor_msgs::PointCloud2>("/front_end_cloud", 1, true);
  ...
  this->crop.setMin(Eigen::Vector4f(-this->crop_length_size_,
                                    -this->crop_width_size_,
                                    -this->crop_height_size_, 1.0));
  this->crop.setMax(Eigen::Vector4f(this->crop_length_size_,
                                    this->crop_width_size_,
                                    this->crop_height_size_, 1.0));
...
void dlio::OdomNode::getParams() {
  ...
  // Keyframes_capacity
  ros::param::param<int>("~dlio/odom/keyframes_capacity",
                         this->keyframes_capacity_, 50);
  ...
  ros::param::param<double>(
      "~dlio/odom/preprocessing/cropBoxFilter/length_size",
      this->crop_length_size_, 1.0);
  ros::param::param<double>("~dlio/odom/preprocessing/cropBoxFilter/width_size",
                            this->crop_width_size_, 1.0);
  ros::param::param<double>(
      "~dlio/odom/preprocessing/cropBoxFilter/height_size",
      this->crop_height_size_, 1.0);
  ...
  // Jump Point
  ros::param::param<bool>("~dlio/odom/preprocessing/jumpPoint/do_jump",
                          this->do_jump_, false);
  ros::param::param<int>("~dlio/odom/preprocessing/jumpPoint/jump_interval",
                         this->jump_interval_, 3);
...
void dlio::OdomNode::publishToROS(
  ...
  Eigen::Matrix4f T_odom = this->T;
  Eigen::Matrix4f T_odom_inverse = T_odom.inverse();
  Eigen::Matrix3f T_odom_rotation = T_odom.block<3, 3>(0, 0);
  Eigen::Vector3f T_odom_translation = T_odom.block<3, 1>(0, 3);
  Eigen::Quaternionf T_odom_orientation(T_odom_rotation);
  pcl::transformPointCloud(*deskewed_scan_t_, *deskewed_scan_t_,
                           T_odom_inverse);

  sensor_msgs::PointCloud2 perception_cloud_msg;
  pcl::toROSMsg(*deskewed_scan_t_, perception_cloud_msg);
  perception_cloud_msg.header.stamp = this->scan_header_stamp;
  perception_cloud_msg.header.frame_id = "wuba_base";
  this->perception_cloud_pub.publish(perception_cloud_msg);

  nav_msgs::Odometry perception_odom_msg;
  perception_odom_msg.header.stamp = this->scan_header_stamp;
  perception_odom_msg.header.frame_id = "map";
  perception_odom_msg.child_frame_id = "wuba_base";
  perception_odom_msg.pose.pose.position.x = T_odom_translation[0];
  perception_odom_msg.pose.pose.position.y = T_odom_translation[1];
  perception_odom_msg.pose.pose.position.z = T_odom_translation[2];
  perception_odom_msg.pose.pose.orientation.w = T_odom_orientation.w();
  perception_odom_msg.pose.pose.orientation.x = T_odom_orientation.x();
  perception_odom_msg.pose.pose.orientation.y = T_odom_orientation.y();
  perception_odom_msg.pose.pose.orientation.z = T_odom_orientation.z();
  this->perception_odom_pub.publish(perception_odom_msg);
...
void dlio::OdomNode::getScanFromROS(
  ...
  pcl::PointCloud<PointType>::Ptr original_scan_filted_(
      boost::make_shared<pcl::PointCloud<PointType>>());
  if (this->do_jump_) {
    int count_point = 0;
    for (auto& point : *original_scan_) {
      if (count_point % this->jump_interval_ == 0) {
        (*original_scan_filted_).push_back(point);
      }
      count_point++;
    }
  }
  ...
  if (this->do_jump_) {
    this->original_scan = original_scan_filted_;
  } else {
...
// void dlio::OdomNode::callbackLivox(
//     const livox_ros_driver2::CustomMsgConstPtr& livox) {
...
void dlio::OdomNode::callbackLivox(
  ...
  // convert custom livox message to pcl pointcloud
  pcl::PointCloud<LivoxPoint>::Ptr cloud(new pcl::PointCloud<LivoxPoint>);
  cloud->reserve(livox->point_num);

  std::vector<std::int64_t> signed_offsets;
  signed_offsets.reserve(livox->point_num);
  std::int64_t min_offset = std::numeric_limits<std::int64_t>::max();

  for (size_t i = 0; i < livox->point_num; i++) {
    std::int64_t offset =
        static_cast<std::int64_t>(livox->points[i].offset_time);
    if (offset >
        static_cast<std::int64_t>(std::numeric_limits<std::int32_t>::max())) {
      offset -= (1LL << 32);
    }
    signed_offsets.push_back(offset);
  ...
  for (size_t i = 0; i < livox->point_num; i++) {
    ...
    p.z = livox->points[i].z;
    p.intensity = livox->points[i].reflectivity;
    p.offset_time = static_cast<std::uint32_t>(signed_offsets[i] - min_offset);
    cloud->push_back(p);
  }
...
void dlio::OdomNode::updateKeyframes() {
  ...
  if (newKeyframe) {
    ...
    if (this->keyframes.size() < this->keyframes_capacity_) {
      this->keyframes.push_back(
          std::make_pair(std::make_pair(this->lidarPose.p, this->lidarPose.q),
                         this->current_scan));
...
void dlio::OdomNode::buildSubmap(State vehicle_state) {
  ...
  if (this->submap_kf_idx_curr != this->submap_kf_idx_prev) {
    ...
    // std::cout << "size: " << this->submap_cloud->points.size() << std::endl;

    // Pause to prevent stealing resources from the main loop if it
    // is running.
    this->pauseSubmapBuildIfNeeded();
...
void dlio::OdomNode::buildKeyframesAndSubmap(State vehicle_state) {
  ...
  std::cout << this->keyframes.size() << " " << this->num_processed_keyframes
            << std::endl;
