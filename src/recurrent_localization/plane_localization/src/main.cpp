...
ros::Subscriber sub_plane_global_localization_;
...
void ReadParam(ros::NodeHandle &nh) {
  nh.param<std::string>("plane_localization/work_mode", work_mode_, "");
  ...
  nh.param<std::string>("plane_localization/localization_mode",
                        localization_mode_, "");
  std::cout << "localization_mode: " << localization_mode_ << std::endl;
  nh.param<int>("plane_localization/cache_size", cache_size_, 10);
  std::cout << "cache_size: " << cache_size_ << std::endl;
  nh.param<int>("plane_localization/localization_fps", localization_fps_, 10);
...
void ViewPlane(std::vector<std::shared_ptr<OctreePlane>> &octree_planes,
  ...
  for (int i = 0; i < octree_planes.size(); i++) {
    ...
    marker_msg.header.frame_id = "map";  // 设置参考系
  ...
  plane_point_cloud_map_msg.header.frame_id = "map";
...
void PlaneLocalization(
  ...
  //遍历所有三对对应平面，计算候选变换矩阵
  clock_t start_find_candidate = clock();
  std::vector<Eigen::Matrix4f> transformation_matrix_vector;
  ...
  clock_t end_find_candidate = clock();
  double find_candidate_time =
      ((double)(end_find_candidate - start_find_candidate) /
       (double)CLOCKS_PER_SEC);
  std::cout << "find_candidate_time: " << find_candidate_time << std::endl;
  ...
  transformation_matrix_points_all_msg.header.frame_id = "map";
  pub_transformation_matrix_points_all_.publish(
      transformation_matrix_points_all_msg);

  clock_t start_rough_verify = clock();
  ...
  clock_t end_rough_verify = clock();
  double rough_verify_time = ((double)(end_rough_verify - start_rough_verify) /
                              (double)CLOCKS_PER_SEC);
  std::cout << "rough_verify_time: " << rough_verify_time << std::endl;

  clock_t start_sort_candicate = clock();
  ...
  clock_t end_sort_candicate = clock();
  double sort_candicate_time =
      ((double)(end_sort_candicate - start_sort_candicate) /
       (double)CLOCKS_PER_SEC);
  std::cout << "sort_candicate_time: " << sort_candicate_time << std::endl;
  ...
  transformation_matrix_points_good_msg.header.frame_id = "map";
    ...
    if (init_localization_) {
      ...
      for (int iter = 0; iter < ndt_max_iteration_; iter++) {
        ...
        for (auto &point : point_cloud_filtered->points) {
          ...
          bool is_distribution = plane_octree_occupy_map_->QueryDistribution(
              query_point, plane_octree_occupy_map_depth_ - 1, query_mean,
              query_covariance);
...
//平面重定位回调函数
void PlaneGlobalLocalizationCallback(std_msgs::Bool::ConstPtr msg) {
  pcl::PointCloud<pcl::PointXYZ>::Ptr point_cloud(
      new pcl::PointCloud<pcl::PointXYZ>);
  ...
  //如果为真使用平面特征进行重定位，如果为假使用建图原点默认值作为重定位值
  if (msg->data == true) {
    //执行初始定位
    //构造scan八叉树
...
void ConstructOctreeMap(float map_range) {
  ...
  //融合分布特性
  plane_octree_occupy_map_->OctreeDistributionExtraction();
  ...
  std::string octree_occupy_binary_name =
      program_path_ + "/data/" + relative_path_ + "/octree_occupy.bin";
  ...
  std::string octree_planes_binary_name =
      program_path_ + "/data/" + relative_path_ + "/octree_planes.bin";
