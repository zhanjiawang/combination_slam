...
bool SCManager::savePolarContexts(const std::string &filename) {
  std::ofstream out(filename, std::ios::binary);
  if (!out.is_open()) return false;

  // 1. 写入 vector 大小
  size_t vec_size = polarcontexts_.size();
  out.write(reinterpret_cast<const char *>(&vec_size), sizeof(size_t));

  std::cout << "polarcontexts_ size: " << polarcontexts_.size() << std::endl;

  // 2. 逐个写入每个矩阵
  for (const auto &mat : polarcontexts_) {
    // 写入行数和列数
    ...
    // 写入矩阵数据（按存储顺序，默认列主序，但读取时同样顺序即可）
    // 注意：Eigen::MatrixXd 的数据是连续存储的
...
bool SCManager::loadPolarContexts(const std::string &filename) {
  ...
  polarcontexts_.reserve(vec_size);
  ...
  for (size_t i = 0; i < vec_size; ++i) {
    ...
    polarcontexts_.emplace_back(std::move(mat));
  }
...
std::pair<double, float> SCManager::compareCurrentAndIndexed(
    int candidate_index) {
  ...
  std::pair<double, float> result{candidate_dist, yaw_diff_rad};
