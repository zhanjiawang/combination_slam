...
- 重定位的三种模式：通过context_localization的/context_global_localization话题，话题为真时使用scancontext进行全局定位（优势->使用八叉树进行多结果验证），话题为假使用建图原点作初始定位值；通过plane_localization的/plane_global_localization话题，话题为真时使用平面特征进行全局定位（同样使用八叉树进行多结果验证），话题为假使用建图原点作初始定位值；通过/initialpose话题人工输入初始定位值
